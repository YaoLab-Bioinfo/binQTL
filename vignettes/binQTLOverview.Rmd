...
author: "Wen Yao and Shizhong Xu"
date: '`r Sys.Date()`'
output: pdf_document
...
is designed to perform QTL analysis with binmap. Several algorithm including
ANOVA and polygenic control were implemented in
the **binQTL** package. In this vignette, we will rely on two simple, 
illustrative example datasets to explain the usage of **binQTL**.
...
par(mar = c(3, 4, 2, 1))
par(mfrow=c(2, 1))
plotQTL(qtl.res.1[, c(1:4, 6)], main="ANOVA", cex.axis=0.6)
plotQTL(qtl.res.2[, c(1:4, 23)], main="binQTL", cex.axis=0.6, ylab=expression(-log[10](p)))
```
...
par(mar = c(3, 4, 2, 1))
par(mfrow=c(2, 1))
plotQTL(qtl.res.3[, c(1:4, 6)], main="ANOVA", cex.axis=0.6)
plotQTL(qtl.res.4[, c(1:4, 26)], main="binQTL", cex.axis=0.6, ylab=expression(-log[10](p)))
```
